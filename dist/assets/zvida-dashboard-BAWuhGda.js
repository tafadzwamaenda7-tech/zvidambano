...
`),"text/csv");l("z-notify",{send:{done:"Sent",toast:"Broadcast sent to all parties"}});l("z-feed-approve",{approve:{done:"Approved",nav:"#listings",toast:"Listing approved at $240/t — James notified"}});l("z-feed-notify",{notify:{done:"Notified",nav:"#deliveries",toast:"Party notified about the delay"}});l("z-rev-882",{review:{target:".js-z-d882",sel:".dsh-badge",to:"IN REVIEW",tone:"blue",nav:"#disputes",done:"In Review",toast:"Resolution panel opened",foot:n("Confirm Decision","primary sm","Dispute resolved — both parties notified",void 0,"z-resolve-882","confirm")}});l("z-rev-879",{review:{done:"In Review",nav:"#disputes",toast:"Resolution panel opened",foot:s("Awaiting evidence from Peter","amber")}});l("z-resolve-882",{confirm:{target:".js-z-d882",to:"RESOLVED",tone:"green",nav:"#disputes",toast:"Dispute resolved — both parties notified",foot:s("Resolved","green")+' <span style="font-size:12px;color:var(--dsh-text-3)">Decision: $50 penalty applied to farmer payout.</span>'}});l("z-list-james",{approve:{to:"APPROVED",tone:"green",nav:"#listings",toast:"Listing approved — James notified",foot:s("Live on marketplace","green")},reject:{to:"REJECTED",tone:"red",nav:"#listings",toast:"Listing rejected — James notified",foot:s("Rejected","red")}});l("z-list-sarah",{approve:{to:"APPROVED",tone:"green",nav:"#listings",toast:"Listing approved — Sarah notified",foot:s("Live on marketplace","green")},reject:{to:"REJECTED",tone:"red",nav:"#listings",toast:"Listing rejected — Sarah notified",foot:s("Rejected","red")}});l("z-list-peter",{approve:{to:"APPROVED",tone:"green",nav:"#listings",toast:"Listing approved — Peter notified",foot:s("Live on marketplace","green")},reject:{to:"REJECTED",tone:"red",nav:"#listings",toast:"Listing rejected — Peter notified",foot:s("Rejected","red")}});l("z-automatch",{match:{done:"Matched",nav:"#deliveries",toast:"Contract created — parties notified"}});l("z-matchall",{all:{done:"Matched",nav:"#deliveries",toast:"5 contracts created — parties notified"}});l("z-pay-882",{release:{sel:".js-due-882",to:"Released",tone:"green",nav:"#payments",toast:"Payment of $4,000 released to James",foot:s("Released to James · Receipt sent","green")},hold:{sel:".js-due-882",to:"Held",tone:"amber",nav:"#payments",toast:"Payment held — reminder scheduled",foot:s("Held · reminder scheduled","amber")}});l("z-pay-883",{release:{sel:".js-due-883",to:"Released",tone:"green",nav:"#payments",toast:"Payment of $3,600 released to Sarah",foot:s("Released to Sarah · Receipt sent","green")},hold:{sel:".js-due-883",to:"Held",tone:"amber",nav:"#payments",toast:"Payment held — reminder scheduled",foot:s("Held · reminder scheduled","amber")}});l("z-pay-884",{release:{sel:".js-due-884",to:"Released",tone:"green",nav:"#payments",toast:"Payment of $4,200 released to Peter",foot:s("Released to Peter · Receipt sent","green")},hold:{sel:".js-due-884",to:"Held",tone:"amber",nav:"#payments",toast:"Payment held — reminder scheduled",foot:s("Held · reminder scheduled","amber")}});const m={control:{id:"control",label:"Control Tower",icon:e.dashboard,title:"Control Tower",sub:"ZVIDA Exchange — live operations",render:()=>`
      ...
      `,`
        ${i({title:"At a Glance",icon:e.spark,body:f([{label:"Today",value:"$4,200"},{label:"This Week",value:"$18,500"},{label:"This Month",value:"$42,300"}])})}
        ...
        ${i({title:"Recently Matched",icon:e.match,body:`
            ${v(e.leaf,"ANON-1 → ANON-B1","Maize 20t · spread $800","+$800","pos",!1,"#matches")}
            ${v(e.leaf,"ANON-3 → ANON-B3","Wheat 15t · spread $600","+$600","pos",!1,"#matches")}
            ${v(e.leaf,"ANON-2 → ANON-B2","Soya 10t · spread $500","+$500","pos",!1,"#matches")}`})}
      `)}
    `},listings:{id:"listings",label:"Listings",icon:e.listings,title:"Listings",sub:"Approval queue",render:()=>`
      ${O([{label:"Pending Approval",badge:4,active:!0},{label:"Active Listings",badge:12},{label:"Rejected",badge:3}],"zlist")}
      ${A(`
        ...
        <div data-tab-group="zlist" data-tab="Active Listings" style="display:none">
          ${i({body:C(["Supplier","Commodity","Qty","ZVIDA price","Status"],[["Tapiwa","Maize","30t","$215/t",s("Live","green")],["Rudo","Soya","12t","$470/t",s("Live","green")],["Farai","Wheat","25t","$395/t",s("Live","green")],["Tendai","Sugar Beans","8t","$640/t",s("Live","green")]],[],["#deliveries","#deliveries","#deliveries","#deliveries"]),flush:!0})}
        </div>
        <div data-tab-group="zlist" data-tab="Rejected" style="display:none">
          ${i({body:C(["Supplier","Commodity","Reason","Date"],[["Blessing","Maize",s("Moisture 16%","red"),"Jul 24"],["Chipo","Soya",s("Unverified GPS","red"),"Jul 20"],["Nomatter","Wheat",s("Duplicate listing","red"),"Jul 18"]]),flush:!0})}
        </div>
      `,`
        ${i({title:"Listing Stats",icon:e.listings,body:f([{label:"Pending",value:"4"},{label:"Active",value:"12"},{label:"Rejected",value:"3"}])})}
    ...
    `},deliveries:{id:"deliveries",label:"Freight Ops",icon:e.deliveries,title:"Freight Ops",sub:"Consignments, weighbridge & payments",render:()=>{const o=H(),a=Z(o),r=o.filter(t=>!["PAID","CANCELLED"].includes(t.status)),d=o.filter(t=>t.status==="PENDING_PAYMENT");return`
      ...
      ${A(`
        ...
        ${r.map(t=>K(t,"admin")).join("")}
      `,`
        ${i({title:"Movement Overview",icon:e.deliveries,body:f([{label:"In transit",value:String(a.inTransit)},{label:"Loading / offloading",value:String(a.loading)},{label:"Awaiting payment",value:String(a.pendingPay)},{label:"Settled",value:String(a.paid)}])})}
        ${i({title:"Payment Releases",icon:e.payments,body:d.length?d.map(t=>v(e.payments,t.ref,`${t.supplier} · ${t.payTerm} · due ${t.due}`,Q(t.amount),"pos",!1,"#payments")).join(""):p("ok","No payments pending — all consignments settled.")})}
        ${i({title:"Vehicle Pool",icon:e.truck,body:o.filter(t=>t.truck).slice(0,4).map(t=>v(e.truck,t.truck,`On ${t.ref} · ${t.driver||"unassigned"}`,t.status==="IN_TRANSIT"?"En route":t.status==="PENDING"?"Idle":"Active","plain",!1,"#deliveries")).join("")||p("info","No vehicles assigned yet.")})}
      ...
      `)}
    `}},disputes:{id:"disputes",label:"Disputes",icon:e.disputes,title:"Disputes",sub:"Resolve quality & weight issues",render:()=>`
      ${O([{label:"Open",badge:2,active:!0},{label:"In Review",badge:1},{label:"Resolved",badge:5}])}
      ${A(`
        ...
        ${i({title:"Dispute resolution",icon:e.disputes,body:`
            ...
            ${M("Resolution notes",_(2,"Explain your decision…"))}
            <div class="dsh-btn-row">${n("Confirm Decision","primary","Dispute resolved — both parties notified",void 0,"z-resolve-882","confirm")}</div>`})}
      `,`
        ${i({title:"Dispute Overview",icon:e.disputes,body:f([{label:"Open",value:"2"},{label:"In review",value:"1"},{label:"Resolved",value:"5"}])})}
        ${i({title:"Resolution Trends",icon:e.scale,body:k([{label:"Quality",pct:60},{label:"Weight",pct:30},{label:"Late delivery",pct:10}])})}
    ...
    `},payments:{id:"payments",label:"Payments",icon:e.payments,title:"Payments",sub:"Escrow & commission",render:()=>`
      ${O([{label:"Pending",badge:3,active:!0},{label:"Processing",badge:2},{label:"Completed",badge:12}])}
      ...
      ${c("Commission Ledger")}
      ${i({body:f([{label:"Today",value:"$4,200"},{label:"This Week",value:"$18,500"},{label:"This Month",value:"$42,300"}])})}
      ${i({title:"Pending Commission",icon:e.scale,body:`
          ${v(e.scale,"Contract #882","Spread realized","+$800","pos",!1,"#reports")}
          ${v(e.scale,"Contract #883","Spread realized","+$900","pos",!1,"#reports")}
          ${v(e.scale,"Contract #884","Spread realized","+$900","pos",!1,"#reports")}`})}
    `},reports:{id:"reports",label:"Reports",icon:e.reports,title:"Reports",sub:"Margin & volume analytics",render:()=>`
      ${i({body:f([{label:"Gross Margin (Month)",value:"$42,300"},{label:"Cash Flow In",value:"$186k"},{label:"Cash Flow Out",value:"$152k"}])})}
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
        ${i({title:"Weekly Margin",icon:e.trendingUp,body:k([{label:"Week 1",pct:42},{label:"Week 2",pct:55},{label:"Week 3",pct:68},{label:"Week 4",pct:100}])})}
        ${i({title:"Cash Flow — This Month",icon:e.wallet,body:k([{label:"Inflows",pct:100},{label:"Outflows",pct:78,alt:!0},{label:"Net",pct:22}])})}
      </div>
      ${c("Top Commodities by Volume")}
      ${i({body:C(["Commodity","Volume","Margin"],[["Maize","420 t","$18,200"],["Soya","180 t","$11,500"],["Wheat","150 t","$7,800"],["Sugar Beans","90 t","$4,800"]],[2],["#payments","#payments","#payments","#payments"]),flush:!0})}
      ${c("Seasonality")}
      ${p("info","Peak volume expected <b>Sep–Nov</b> (harvest season). Plan truck capacity ahead.")}
      <div class="dsh-btn-row">${D("Export Tax Compliance File","outline","exportTax","","Tax file exported")}${D("Export Carbon Footprint Report","outline","exportCarbon","","Carbon report exported")}</div>
    `},marketplace:{id:"marketplace",label:"Marketplace",icon:e.shop,title:"Marketplace",sub:"E-commerce order oversight",render:()=>{const o=J(),a=o.filter(t=>!["DELIVERED","PAID","CANCELLED","ESCALATED"].includes(t.status)).length,r=o.filter(t=>t.status==="DELIVERED").reduce((t,g)=>t+g.total,0),d=o.filter(t=>t.status==="ESCALATED").length;return`
      ${b([{label:"Open Orders",value:a,icon:e.orders,delta:"Awaiting fulfilment",up:!0,spark:[2,3,4,3,5,4,Math.max(a,2)],foot:"Across the marketplace",open:"#marketplace"},{label:"GMV (Delivered)",value:w(r),icon:e.trendingUp,delta:"Completed orders",up:!0,spark:[40,60,90,80,120,140,Math.max(r/10,10)],foot:"Sellers paid via NET_7",open:"#marketplace"},{label:"Escalated",value:d,icon:e.alert,delta:"Needs resolution",up:!1,spark:[1,0,1,0,1,0,Math.max(d,1)],foot:"Resolution desk",open:"#disputes"}])}
      ${p("info","Monitor farmer orders end-to-end: seller confirms, driver delivers, payment releases. Escalate when buyers and sellers disagree.")}
    ...
    `}}};function x(o,a,r,d,t,g,$,u,h,N,T,E){const I=h===0?"green":h>30?"red":"amber",G=h===0?"New listing":`Spoilage: ${h} days`;return`<div class="dsh-item" style="display:flex;gap:16px">
    ${te(a,"sm",r)}
    <div style="flex:1;min-width:0">
      ...
      <div class="dsh-item-meta">${t} · ${g} · Moisture ${$}% · Reserve <b>$${u}/t</b></div>
      ${N?`<div style="font-size:12px;color:var(--dsh-warn);margin-top:4px">${ae(e.clock)} ${N}</div>`:""}
      <div class="dsh-item-foot">
        ${M("ZVIDA price",`<span style="display:flex;align-items:center;gap:8px"><span>$</span><input class="dsh-input" value="${T}.00" style="max-width:110px" /></span>`,`Suggested spread: $${T-u}/t`)}
        <div class="dsh-btn-row">${n("Approve","success",`Listing approved — ${o} notified`,void 0,E,"approve")}${n("Reject","danger","Listing rejected",void 0,E,"reject")}</div>
      </div>
    </div>
  ...
  </div>`}function S(o,a,r,d,t,g,$,u,h){return`<div class="dsh-queue-card">
    <span class="dsh-queue-rank">#${o}</span>
    <div class="dsh-queue-body">
      <div class="dsh-queue-title">${a} — ${$} to ${u} · ${h}km</div>
  ...
  </div>`}function z(o,a,r,d,t,g,$,u){return`<div class="dsh-item" data-open="#deliveries">
    <div class="dsh-item-top">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-top:12px">
      ...
      <div><div style="font-size:10.5px;font-weight:650;text-transform:uppercase;letter-spacing:.05em;color:var(--dsh-text-3)">Due</div><div class="${u}" style="font-size:14px;font-weight:700;margin-top:2px">${$}</div></div>
    </div>
    <div class="dsh-item-foot">
      ${n("Release Early","success sm",`Payment of ${r} released`,void 0,`z-pay-${u.slice(-3)}`,"release")}
      ${n("Hold","ghost sm","Payment held",void 0,`z-pay-${u.slice(-3)}`,"hold")}
      ${n("View Contract","outline sm","Opening contract","#deliveries")}
    </div>
  </div>`}V({name:"Admin",roleLabel:"ZVIDA",company:"ZVIDA Brokerage",initials:"Z",accent:"#2563eb",accentHover:"#1d4ed8",accentLight:"#eff6ff",accentRgb:"37, 99, 235",gradientEnd:"#60a5fa",pages:[m.control,m.marketplace,m.listings,m.matches,m.deliveries,m.disputes,m.payments,m.reports]});
